...
def drop_slivers(F, min_side=1.2, min_area=1.5):
    """Facades too small to be a wall — a roofer solid's step risers, jamb returns and
    triangulation crumbs. On the Net Loft 106 of 170 "walls" are under 1.5 m on a side; drawn as
    rectangles they stand around the building as isolated panels with nothing either side of them
    (Kaveh, 2026-09-06). Their neighbours already close the envelope, so dropping them removes
    clutter, not surface. Returns (kept, dropped)."""
    keep, gone = [], []
    for f in F:
        w, h = f["s1"] - f["s0"], f["t1"] - f["t0"]
        (keep if (min(w, h) >= min_side and w * h >= min_area) else gone).append(f)
    return keep, gone


def outer_walls(F, ring, tol=1.5, base_z=None):
    """Only the facades that lie ON the building's outline — the ones a person can see.

    A roofer solid carries interior planes: partitions, light-well faces, the risers between roof
    levels. Measured on the Net Loft, 77 of 89 wall planes sit more than 1 m inside the footprint.
    The pilot never showed them because it only ever DREW a wall a photo had painted; drawing every
    plane put walls in mid-air with nothing either side of them (Kaveh, 2026-09-06). A facade is
    kept when its midpoint is within `tol` of the outline. Returns (kept, inner).
    """
    import numpy as np
    from shapely.geometry import Point, Polygon
    edge = Polygon(np.asarray(ring)).exterior
    keep, inner = [], []
    for f in F:
...
def close_corners(F, tol=0.75):
    """Extend each facade's box sideways to where its plane MEETS its neighbours' planes.

    A roofer solid's walls do not always reach each other: measured on railspur, adjacent walls
    stop 0.15-0.50 m apart, and once each wall is drawn as its own rectangle those gaps read as
    bright slits down the corners (Kaveh, 2026-09-06). Two vertical planes meet in a line; if a
    facade's end sits within `tol` of that line, the end moves onto it. Nothing moves further
    than `tol`, so a wall that genuinely ends in mid-air (a gable return, a setback) stays where
    the survey put it.
    """
    import numpy as np
    for f in F:
        for side in ("s0", "s1"):
            end = f[side]; best = None
            p_end = f["p0"] + f["u"] * end                      # the wall's end point, in plan
            for gf in F:
                if gf is f: continue
                d = np.cross(f["n"][:2], gf["n"][:2])           # parallel planes never meet
                if abs(d) < 0.15: continue
                # intersect the two vertical planes:  n.x = n.p0  for each
                A = np.array([f["n"][:2], gf["n"][:2]])
                b = np.array([f["n"][:2] @ f["p0"][:2], gf["n"][:2] @ gf["p0"][:2]])
                try: xy = np.linalg.solve(A, b)
                except np.linalg.LinAlgError: continue
                s_hit = float((np.array([xy[0], xy[1], f["p0"][2]]) - f["p0"]) @ f["u"])
                if abs(s_hit - end) > tol: continue             # too far: not this corner
                # the neighbour must actually be there: its own box must contain the meeting point
                s_on_g = float((np.array([xy[0], xy[1], gf["p0"][2]]) - gf["p0"]) @ gf["u"])
                if not (gf["s0"] - tol <= s_on_g <= gf["s1"] + tol): continue
                if best is None or abs(s_hit - end) < abs(best - end): best = s_hit
            if best is None: continue
            # never let a snap invert or collapse the wall: the Net Loft's 170 coplanar planes
            # offer a corner candidate for almost every end, and an unguarded snap turned whole
            # facades inside out (0 facades survived).
            lo, hi = (best, f["s1"]) if side == "s0" else (f["s0"], best)
            if hi - lo >= max(0.3, 0.5 * (f["s1"] - f["s0"])): f[side] = best
    return F
